...
Outboxer separates generic routing/lifecycle data from backend-specific publish
options. Generic fields remain normal table columns because they participate in
collection, locking, deletion, ownership, and operator queries. Backend-specific
fields live in one JSON object so each backend can evolve without adding more
top-level event-table columns.
...
- `destination` remains a generic routing column. It is the Pub/Sub topic when
  the resolved target is Pub/Sub, and the SQS queue URL when the resolved target
  is SQS. It is intentionally not part of `options` because collection fairness,
  destination ownership, and operational queries all use resolved destinations.
- `options` is optional event metadata. If the configured options column is
  absent or `NULL`, Outboxer treats it as `{}`. If it is present, it should be a
  JSON object.

The canonical schema removes the old backend metadata columns. Outboxer must not
read `ordering_key` or `attributes` as fallback columns after the options change.
Existing tables must migrate those values into `options` before running this
version.

The default options column is `options`, configurable with `EVENT_OPTIONS`. The
old `EVENT_ORDERING_KEY` and `EVENT_ATTRIBUTES` settings are removed; no
deprecated aliases are kept before `1.0`.

### Backend options

Only the options for the selected backend are interpreted. Unknown keys are
ignored and reserved for future use. Do not infer `target` from the presence of a
backend section in `options`; routing still uses the `target` column and enabled
...
Supported keys for the first options implementation:
...
Options validation runs after route resolution and only for the selected backend.

- Missing `options`, missing backend section, and `null` backend section are
...
event will not make Outboxer interpret them differently. They are therefore
classified under the existing poison categories, not as routing failures:

- malformed attributes/options are P5;
- invalid ordering/group options are P6.
...
- **S2 — Ordered queues keep their order**, within a batch and across consecutive
  batches. For Pub/Sub ordering keys from `options.pubsub.orderingKey` and SQS
  FIFO message groups from `options.sqs.messageGroupId`, never have a later event
  in flight before the earlier event's outcome is known.
...
- The caller's `Get(ctx)` context bounds how long Outboxer waits for a result,
  ...
  process.
- For Pub/Sub ordering keys from `options.pubsub.orderingKey`, a sender must keep
  at most one event in flight per key. It publishes one event, flushes, waits for
  its result, and only then publishes that key's next event. Different ordering
  keys may run concurrently, and unordered events may still use normal async
  batching.
...
- For ordering keys from `options.pubsub.orderingKey`, a publish failure pauses
  the key. The sender must call `ResumePublish(key)` before future sends for that
  key, and must keep the failed event plus that key's later events out of `done`.
...
attributes have been sanitized to strings, documented provider attribute limits
apply. The attribute container itself must still be an object; a non-object
`attributes` value is malformed options and therefore poison.
...
- P4 if data is empty and there are no Pub/Sub attributes and no
  `options.pubsub.orderingKey`. Provider docs are inconsistent about whether an
  ordering key alone is acceptable, so an ordering-key-only Pub/Sub message is not
  local poison; publish it in isolation before classifying.
...
- Pub/Sub `options.pubsub.orderingKey` length or character content unless a
  documented provider limit is encoded. A permanent backend reject for an
  isolated event may still become P6.
...
- **P4 — empty/invalid payload** (Pub/Sub message with no
  data/attributes/ordering key after the conservative local check or isolated
  backend proof; SQS empty body or body containing unsupported characters).
- **P5 — malformed options or over-limit attributes.**
- **P6 — invalid ordering key / message group** (malformed or over-limit
...
- **R9 — ordering key paused** (Pub/Sub `FailedPrecondition` after a prior
  `options.pubsub.orderingKey` failure) → `ResumePublish` + the prior cause
  resolving.
...
- Group ordered events by `options.pubsub.orderingKey` — [S4]
...
- Ordered events: group by `options.pubsub.orderingKey`; for each key publish one event,
...
| S4 | no sender-side ordered-group cap; selected batch composition bounds sequential work |
...
- Remove: `BATCH_WORKERS`, `BATCH_MAX_SEQUENTIAL`, `EVENT_ORDERING_KEY`,
  `EVENT_ATTRIBUTES`.
