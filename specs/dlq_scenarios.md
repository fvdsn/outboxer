...
| DLQ-PAYLOAD-10 | Original event has legacy `ordering_key` or `attributes` columns. | Legacy columns may appear in `original_event` because the base row is preserved, but they do not affect resolved backend options. |
...
| DLQ-POISON-07 | SQS FIFO `options.sqs.messageGroupId` is invalid. | Event is inserted into DLQ and deleted from the outbox table; no provider call is made. |
