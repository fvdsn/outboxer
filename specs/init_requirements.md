...
- `CREATE TABLE IF NOT EXISTS <EVENT_TABLE>` with every **configured** column,
  using the configured column names. The rule is:

  > Create every configured optional column, regardless of whether the current
  > backend/feature configuration requires it.

  This keeps the schema predictable — column inclusion no longer depends on
  backend flags or `MAX_EVENT_AGE_MS`. An optional column is omitted only when it
  is explicitly disabled (`EVENT_OPTIONS=disabled` / `--event-options=disabled`,
  which resolves to an empty internal name). An empty name must never be emitted
  ...
  - `<EVENT_TARGET>` `text` (nullable) — unless disabled.
  - `<EVENT_DESTINATION>` `text` (nullable) — unless disabled.
  - `<EVENT_TIMESTAMP>` `timestamptz` (nullable) — unless disabled.
  - `<EVENT_OPTIONS>` `jsonb` (nullable) — unless disabled.
  - Only `id` and `payload` are `NOT NULL`; the rest are nullable so the relay's
    "optional columns may be absent" semantics still hold.
