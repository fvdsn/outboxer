...
| REAL-OK-03 | `per_route_ordered`, SQS-only standard queue, 100 events split across 10 queue URLs, 10 per queue, default `COLLECT_BATCH_TARGET=2500`. | Selects all 100 events; each queue receives one batch of 10; the global SQS semaphore is respected across queues; all IDs are `done`. |
...
| REAL-OK-05 | `per_route_ordered`, Pub/Sub-only unordered, 100 events split across 10 topics, 10 per topic, default `COLLECT_BATCH_TARGET=2500`. | Selects all 100 events; one cached publisher per topic; each topic is flushed; all IDs are `done`; no topic receives another topic's messages. |
